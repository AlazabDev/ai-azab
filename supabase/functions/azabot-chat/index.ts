...
function foundryConfig() {
  ...
  if (!endpoint || !apiKey || !agentId) {
    throw new Error("Microsoft Foundry agent is not configured (endpoint / key / agent id missing).");
  }
  return { endpoint, apiKey, agentId };
}

async function foundry(path: string, init: RequestInit = {}) {
  const { endpoint, apiKey } = foundryConfig();
  ...
  if (!res.ok) {
    console.error(`Foundry ${path} failed [${res.status}]: ${text}`);
